...
//		System.out.println("Just created " + renderables.size() + " renderables");
	...
	/**
	 * Returns a list of markers for a Query Layer model object 
	 * 
	 * @param mapLayer 
	 * @param queries
	 * @return
	 */
	public static List<Renderable> getOfferingMarkers(MapLayer mapLayer, 
			List<Query> queries) 
	{
		List<Renderable> markers = new ArrayList<Renderable>();
		for (Query query : queries) {
			if (query instanceof QuerySos) {
				QuerySos querySos = (QuerySos)query;
				
				SensorOffering offering = querySos.getSensorOffering();
				Service service = querySos.getProvenance().getService();
				
				// load offering if needed 
				...
				if (isRegion(offering)) {
					...
					SensorOfferingRegion region = 
							new SensorOfferingRegion(offering, sector, 
									regionAttributes);
					// set the service for the marker 
					region.setService(query.getProvenance().getService());
					// update attributes appropriately
					// (must happen after setting the service)
					setAttributesFromLayer(mapLayer, region);
				 ...
				 */
					...
					// construct the offering placemark 
					SensorOfferingPlacemark marker = 
							new SensorOfferingPlacemark(offering, getCentralPosition(offering), 
									placemarkAttributes);
					// set the service for the marker 
					marker.setService(query.getProvenance().getService());
					// update attributes appropriately
					// (must happen after setting the service)
					setAttributesFromLayer(mapLayer, marker);

					markers.add(marker);					
				}
			} else {
				log.warn("Query was not SOS related, unexpected: " + query);
			}
	 ...
	 * @return
	 */
	public static List<Renderable> getOfferingMarkers(QueryLayer queryLayer) {
		List<Renderable> markers = new ArrayList<Renderable>();
		for (Query query : queryLayer) {
			if (query instanceof QuerySensorOffering) {
				QuerySensorOffering querySensorOffering = 
					(QuerySensorOffering)query; 
				SensorOffering offering = 
					querySensorOffering.getSensorOffering();
				...
				if (isRegion(offering)) {
					...
					// update attributes appropriately
					// (must happen after setting the service)
					setAttributesFromLayer(queryLayer, region);
				 ...
				 */
					...
					// update attributes appropriately
					// (must happen after setting the service)
					setAttributesFromLayer(queryLayer, marker);
